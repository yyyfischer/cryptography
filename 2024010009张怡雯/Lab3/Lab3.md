...
**答案：** ____6c73d5240a948c86981bc294814d_______
...
其中 $k$ 是随机生成的内容密钥，$k_{\text{root}}$ 是根节点密钥。由于所有播放器都有 $k_{\text{root}}$，所有播放器均可解密电影。**密钥撤销：**  
...
**答案：**
Adv(prg)[A,G]=|Pr[A(G(k))=1]-Pr[A(G(r))=1]|

**(b)** 解释 $\mathrm{Adv}_{\mathrm{PRG}}[A, G]$ 接近 1 和接近 0 分别意味着什么。

**答案：**
接近1意味着认为A可预测，不是真随机值；反之，接近0意味着认为A可预测，是真随机值`
...
2/3；
根据全概率公式：
Pr[A(G(k))=1] = Pr[MSB(G(k))=1] * Pr[A(G(k))=1 | MSB(G(k))=1] + Pr[MSB(G(k))=0] * Pr[A(G(k))=1 | MSB(G(k))=0]

当 MSB(G(k))=1 时，A(G(k)) 必然等于 1，所以条件概率为 1。
当 MSB(G(k))=0 时，A(G(k)) 必然等于 0，所以条件概率为 0。
...
代入公式：
Pr[A(G(k))=1] = (2/3)*1 + (1/3)*0 = 2/3

**(b)** 计算 $\Pr[A(r) = 1]$，其中 $r \xleftarrow{R} \{0,1\}^n$。

**答案：**
 1/2;r 是随机的 n 位字符串。
r 的最高有效位等概率为 0 或 1，所以 Pr[MSB(r)=1] = 1/2。
A(r)=1 当且仅当 MSB(r)=1
...
**答案：**
不安全，安全的PRG要求优势小到可以忽略，1/6是一个不可忽略的常数，攻击者可以轻易分辨是真随机还是伪随机
...
**答案：**
对于任意明文 m 和密文 c，密钥 k 从密钥空间中均匀随机选取时，加密函数 E(k,m) 等于 c 的概率等于 1 除以密文空间的大小。

**(b)** 对于 OTP，证明对任意 $m \in \mathcal{M}$ 和 $c \in \mathcal{C}$，恰好存在多少个密钥 $k$ 使得 $E(k, m) = c$？

**答案：**
1个，对于一次一密，加密方式是 c = m XOR k。给定 m 和 c，方程 m XOR k = c 有唯一解：k = m XOR c。因此恰好存在 1 个密钥使得 E(k, m) = c。

**(c)** 利用 (a) 和 (b) 的结论，完成 OTP 具有完美保密性的证明。

**答案：**
证明：由 (b) 得，对任意 m 和 c，有 Pr[E(k,m)=c] = Pr[k = m XOR c]。由于密钥 k 在 {0,1}^n 上均匀随机，每个具体密钥出现的概率是 1 / 2^n。因此 Pr[E(k,m)=c] = 1 / 2^n。而密文空间大小也是 2^n，所以这个概率等于 1 除以密文空间大小。又由 (a) 中完美保密性的定义，一次一密具有完美保密性。
...
**答案：**
证明：语义安全的意思是：攻击者没法用不可忽略的优势区分两个不同明文的密文。
现在题目假设存在一个算法 A，总能从密文里直接算出明文的最后一个比（LSB）。
那攻击者就可以选两条明文，一条最后一位是 0，另一条最后一位是 1。看到密文后，用 A 算出最后一位：如果是 0 就猜用的是第一条明文，是 1 就猜用的是第二条。
...
**答案：**
攻击者 B ：选两条消息 m0 和 m1，比如 m0 = 末尾是 0，m1 = 末尾是 1（其他位随便）。把 (m0, m1) 发给挑战者，挑战者随机选一个 b，返回密文 c = E(k, mb)。
B 把 c 丢给算法 A，A 会返回明文的最后一位。
如果 A 说最后一位是 0，B 就猜 b=0；如果 A 说最后一位是 1，B 就猜 b=1。
因为 A 每次都对，所以 B 每次也都猜对。


**(c)** 计算 $\mathrm{Adv}_{\mathrm{SS}}[B, \mathbb{E}]$ 的值。

**答案：**
1/2；因为B 总是猜对，所以猜对的概率 = 1，
Adv=  1 - 1/2 = 1/2
...
**答案：**
证明：在一次性密码本里，密钥 k 是从 {0,1}^n 里均匀随机选的。固定 m0 后，c = k XOR m0 其实就是把 k 的每个比特和 m0 的对应比特做异或。
因为 k 是均匀随机的，所以 k XOR m0 也是均匀随机的。


**(b)** 证明在 EXP(1) 中，密文 $c = k \oplus m_1$ 在 $\{0,1\}^n$ 上也是均匀分布的。

**答案：**
证明：不管 m1 是什么，k 均匀随机时，k XOR m1 也是均匀随机的。
所以 c 的分布也是均匀分布。
...
**答案：**
由(a) 和 (b)得：密文 c 都是 {0,1}^n 上的均匀分布。一个是从 k XOR m0 得到的，另一个是从 k XOR m1 得到的，但它们最后形成的概率分布完全一样。
所以两个实验的密文分布是相同的。
...
**答案：**
证明：Adv_ss[A, OTP] = | Pr[ A(EXP(0)的密文) = 1 ] - Pr[ A(EXP(1)的密文) = 1 ] |。但由 (c) 可知，EXP(0) 和 EXP(1) 中的密文分布一模一样。
既然密文在两种情况下长得完全一样（统计上无法区分），那么任意攻击者 A 输出 1 的概率在两个实验里也完全相等。
所以两个概率相减为 0，优势 = 0。
这说明 OTP 是语义安全的。
