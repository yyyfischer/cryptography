...
**答案：** _6c73d3240a948c86981bc2808548__________
...
PRG 优势的定义是统计测试
A
区分 PRG 输出和真随机串的概率差的绝对值，公式为：
Adv 
PRG
​
 [A,G]=∣Pr[A(G(k))=1]−Pr[A(Un )=1]∣
```
...
**答案：**
$\mathrm{Adv}{\mathrm{PRG}}[A， G]$ 接近 1：说明区分器 A 能以极高概率区分 G 输出的伪随机串和真随机串，意味着 G 不是安全的伪随机数生成器（PRG），其输出的伪随机性被攻破。 $\mathrm{Adv}{\mathrm{PRG}}[A， G]$ 接近 0：说明区分器 A 无法有效区分伪随机串和真随机串，G 输出的串在统计上与真随机串不可区分，G 是安全的 PRG。
...
**答案：**
$$ \mathrm{Adv}_{\mathrm{PRG}}[A， G] = \left|\dfrac{2}{3} - \dfrac{1}{2} \对了|= \boldsymbol{\dfrac{1}{6} \约0.1667} $$
```
...
**答案：**
对任意明文$m_0，m_1 \in \mathcal{M}$，任意密文c∈C，满足： $$ \underset{k \xleftarrow{R} K}{\Pr}[E（k，m_0）=c] = \underset{k \xleftarrow{R} K}{\Pr}[E（k，m_1）=c] $$
```
...
**答案：**
OTP 的加密规则为：E(k,m)=m⊕k（⊕ 表示逐位异或）。要满足 E(k,m)=c，即 m⊕k=c，可以推出：
k=m⊕c
```
...
**答案：**
对于任意明文 m 和密文 c，Pr[E(k,m)=c]=∣K∣满足 E(k,m)=c 的密钥数量​=∣K∣1​。
由于该概率与明文 m 的选择无关，因此对于任意 m0​,m1​∈M，都有：
Pr[E(k,m0​)=c]=∣K∣1​=Pr[E(k,m1​)=c]这满足完美保密性的定义，故 OTP 具有完美保密性。
```
...
**答案：**
存在高效算法A能推断明文的最低有效位，可构造攻击者B利用A使语义安全游戏的优势不可忽略，故E不是语义安全的
```
...
**答案：**
选择m0​=0、m1​=1；收到c后用A得到LSB(mb​)，若为 0 则猜b=0，若为 1 则猜b=1
```
...
**答案：**
已知密钥k是从{0,1}n中均匀随机选取的，即对于任意k0​∈{0,1}n，有Pr[k=k0​]=2n1​。
对于任意目标密文c0​∈{0,1}n，我们有：
Pr[c=c0​]=Pr[k⊕m0​=c0​]=Pr[k=c0​⊕m0​]因为k是均匀分布的，所以Pr[k=c0​⊕m0​]=2n1​。
因此，c在{0,1}n上是均匀分布的。
```
...
**答案：**
密钥k是从{0,1}n中均匀随机选取的，对于任意目标密文c0​∈{0,1}n：
Pr[c=c0​]=Pr[k⊕m1​=c0​]=Pr[k=c0​⊕m1​]=2n1​因此，c在{0,1}n上是均匀分布的。
```
...
**答案：**
EXP(0)中密文c的分布是{0,1}n上的均匀分布；
EXP(1)中密文c的分布也是{0,1}n上的均匀分布。
两个实验的密文分布完全相同，因此EXP(0)和EXP(1)的密文分布是相同的。
```
...
**答案：**
语义安全的优势定义为：
Advss​[A,OTP]=∣Pr[A(k⊕m0​)=1]−Pr[A(k⊕m1​)=1]∣由 (c) 可知，k⊕m0​和k⊕m1​的分布完全相同，因此对于任意攻击者A，有：
Pr[A(k⊕m0​)=1]=Pr[A(k⊕m1​)=1]代入优势公式得：
Advss​[A,OTP]=∣p−p∣=0因此，OTP 是语义安全的。
```
