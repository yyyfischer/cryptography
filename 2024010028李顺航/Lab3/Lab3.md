...
**答案：** 6c73d5240a948c86981bc2949d5d
...
**答案：AdvPRG​[A,G]=∣Prk​[A(G(k))=1]−Prr​[A(r)=1]∣
...
**答案：优势接近 1：攻击者 A 能有效区分伪随机序列与真随机序列
       优势接近 0：攻击者 A 无法区分，PRG 安全
...
**答案：对任意明文 m、密文 c，Prk​[E(k,m)=c] 与 m 无关。
...
**答案：OTP 加密为 c=m⊕k，对任意 m,c，满足 k=m⊕c 的密钥唯一。
       因此 Prk​[E(k,m)=c]=1/∣K∣，与 m 无关，满足完美保密定义。
```
...
**答案：若攻击者总能从密文得到 LSB(m)，则可区分任意两条 LSB 不同的明文，违反语义安全 “密文不泄露任何明文信息” 的要求，故方案不是语义安全的。
...
**答案：选择 m0​=0n，m1​=1n
收到密文 c 后，用 A 得到 LSB(m)，将结果作为对 b 的猜测
...
**答案：EXP(0) 与 EXP(1) 密文分布完全相同，均为均匀分布。
...
**答案：对任意攻击者 A：
Pr[A(c0​)=1]=Pr[A(c1​)=1]
AdvSS​[A,OTP]=∣Pr[A(c0​)=1]−Pr[A(c1​)=1]∣=0
答案：0
