...
**答案：** 6c73d5240a948c86981bc2808540
...
**答案：**
 \boldsymbol{\text{Adv}_{\text{PRG}}[A, G] = \left| \Pr_{k \leftarrow K}[A(G(k)) = 1] - \Pr_{r \leftarrow \{0,1\}^n}[A(r) = 1] \right|}

**(b)** 解释 $\mathrm{Adv}_{\mathrm{PRG}}[A, G]$ 接近 1 和接近 0 分别意味着什么。

**答案：**
接近1：区分器易区分伪随机序列与真随机序列，PRG安全性差；接近0：区分器几乎无法区分，PRG安全性好。
...
**答案：**
 对于任意明文m和密文c，\boldsymbol{\Pr[k \mid E(k,m)=c] = \Pr[k]}
...
结合(a)的定义与(b)的结论，通过贝叶斯公式可证\Pr[m \mid c] = \Pr[m]，故OTP具有完美保密性。
...
存在攻击者能以不可忽略的优势区分明文加密结果，故  E  不是语义安全的
...
选择  \text{LSB}  不同的两个明文，利用  A  得到  \text{LSB}  后判断  b
